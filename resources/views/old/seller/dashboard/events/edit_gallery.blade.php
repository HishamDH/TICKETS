...
<div class="glassmorphism p-10 rounded-lg shadow-lg grid grid-cols-1 md:grid-cols-2 gap-6">
    <div class="grid grid-cols-4 gap-3">
        ...
        <div id="imageViewerContainer" class="mb-6 col-span-3">
            <img height="190px" style="max-height: 190px;" id="imageView" src="{{ Storage::url($event->image) }}" alt="صورة الفعالية" class="rounded-lg shadow-lg object-cover">
        </div>
    </div>
    <!-- <div>
        <img src="{{ Storage::url($event->image) }}" alt="Restaurant Image" class="rounded-lg shadow-lg w-full h-auto object-cover">
    </div> -->
    <div>
        <p class="text-gray-600 mb-2"><strong>Location:</strong> {{ $event->name }}</p>
        <p class="text-gray-600 mb-2"><strong>event:</strong> {{ $event->location }}</p>
        <p class="text-gray-600 mb-2"><strong>Average Price:</strong> SAR {{ number_format($event->hour_price, 2) }}</p>
        <p class="text-gray-600 mb-2"><strong>Rating:</strong> ⭐ {{ number_format($event->rating, 1) }} / 5</p>
        <p class="text-gray-600 mb-4"><strong>Status:</strong>
            <span class="{{ $event->status === 'open' ? 'text-green-600' : 'text-red-600' }}">
