<div class="relative" wire:poll.5s>
    <!-- Bell Button -->
    <button class="relative text-slate-600 hover:text-orange-500" wire:click="$toggle('showList')">
        ...
        @if($unreadCount > 0)
            <span class="absolute -top-1 -right-1 flex items-center justify-center h-4 w-4 text-[10px] font-bold rounded-full bg-red-500 text-white">
                {{ $unreadCount }}
            </span>
        @endif
    </button>
    ...
    @if ($showList ?? false)
        <div class="absolute right-0 mt-2 w-80 bg-white border border-gray-200 rounded-lg shadow-lg z-50">
            <div class="p-4 border-b text-sm font-bold text-slate-700">📬 إشعاراتك</div>
            <ul class="max-h-64 overflow-y-auto divide-y">
                ...
                @empty
                    <li class="p-3 text-gray-500 text-sm">لا توجد إشعارات حاليًا.</li>
                @endforelse
            </ul>
        </div>
    @endif
