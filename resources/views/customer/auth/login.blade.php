            ...
            <div class="container mx-auto px-4" bis_skin_checked="1">
                <div class="max-w-2xl mx-auto" bis_skin_checked="1" style="opacity: 1; transform: none;">
                    <div class="bg-white/70 backdrop-blur-xl rounded-2xl shadow-xl p-8 md:p-10 border" bis_skin_checked="1">
                        ...
                        @enderror
                        <form action="{{route('customer.singin',array_merge(['guard'=>'customer'],request()->all()))}}" method="POST" class="space-y-6">
                            @csrf
                        ...
                        <div class="text-center mt-4">ليس لديك حساب؟ <a href="{{ route('customer.register',request()->all()) }}" class="text-orange-500 font-bold">طلب الانضمام</a></div>
                    </div>
                </div>
            </div>
        </div>
